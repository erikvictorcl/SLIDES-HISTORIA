...
import { activitiesImages } from '../data/materialsData';
...
export const ActivitiesCarousel: React.FC = () => {
  ...
  const prev = () => {
    setActiveIndex((current) => (current === 0 ? activitiesImages.length - 1 : current - 1));
  };

  const next = () => {
    setActiveIndex((current) => (current === activitiesImages.length - 1 ? 0 : current + 1));
  };
  ...
  return (
    <section id="atividades" className="w-full py-8 sm:py-10 bg-[#FAF8F5] border-t border-slate-200/60">
      <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-6 sm:mb-8">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-[#1B3B57] tracking-tight font-display">
            Atividades prontas para trabalhar História no ritmo do ENEM
          </h2>
          <p className="text-sm sm:text-base text-slate-600 mt-2 font-normal">
            Exercícios para aplicar e reforçar o conteúdo em sala.
          </p>
        </div>

        {/* Carrossel Grande de Documentos / Folhas */}
        <div 
        ...
        >
          {/* Desktop: Central em destaque com laterais visíveis */}
          <div className="hidden md:grid md:grid-cols-3 gap-6 items-center max-w-5xl mx-auto">
            {[-1, 0, 1].map((offset) => {
              const targetIndex = (activeIndex + offset + activitiesImages.length) % activitiesImages.length;
              const activity = activitiesImages[targetIndex];
              ...
              return (
                <div
                  ...
                  className={`transition-all duration-300 rounded-2xl overflow-hidden ${
                    isCenter
                      ? 'scale-100 z-10 shadow-xl shadow-slate-900/10 border-2 border-[#244E73]/40 cursor-default'
                ...
                >
                  <div className="relative aspect-[3/4] w-full bg-white flex items-center justify-center p-4">
                    {activity.src ? (
                      <img
                        src={activity.src}
                        alt={activity.alt || activity.label}
          ...
          <div className="md:hidden max-w-sm mx-auto">
            {(() => {
              const activity = activitiesImages[activeIndex];
              return (
